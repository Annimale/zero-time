  ...
  applyFilters(): void {
    ...
    this.filteredWatches = this.watches.filter((watch: Watch) => {
      ...
      const matchesSearchTerm =
        brandNameLower.includes(searchTermLower) ||
        watch.model.toLowerCase().includes(searchTermLower) ||
        watch.price.toString().includes(searchTermLower);

      const matchesBrand =
        this.filters.selectedBrands.size === 0 ||
        this.filters.selectedBrands.has(watch.brandID.toString());

      const matchesPrice =
        watch.price >= this.filters.currentPrice &&
        watch.price <= this.filters.priceRange.max;
